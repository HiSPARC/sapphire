...
class Master:
    ...
    def reconstruct_direction(self):
        for station_id, station_group in enumerate(self.station_groups):
            reconstruction = DirectionReconstruction(self.data,
                                '/reconstructions_%s' % station_group,
                                overwrite=True)
            group = self.data.getNode(station_group)
            reconstruction.reconstruct_angles(group,
                                              offsets=self.detector_offsets[station_id])
...
if __name__ == '__main__':
    ...
    master = Master('master-single.h5')
    master.main()
