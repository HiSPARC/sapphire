...
if __name__ == '__main__':
    ...
    simulation = KascadeLdfSimulation(cluster, data, '/ldfsim/gauss_10', R=60, N=N, gauss=0.1, trig_threshold=0.9)
    simulation.run(max_theta=pi / 3)

    simulation = KascadeLdfSimulation(cluster, data, '/ldfsim/gauss_20', R=60, N=N, gauss=0.2, trig_threshold=0.8)
    simulation.run(max_theta=pi / 3)

    simulation = KascadeLdfSimulation(cluster, data, '/ldfsim/poisson', R=60, N=N, use_poisson=True)
    simulation.run(max_theta=pi / 3)

    simulation = KascadeLdfSimulation(
        cluster,
        data,
        '/ldfsim/poisson_gauss_20',
        R=60,
        N=N,
        use_poisson=True,
        gauss=0.2,
