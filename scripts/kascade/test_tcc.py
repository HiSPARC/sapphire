...
def plot_core_positions(data):
    ...
    graph.add_pin(r'$T_{CC} \geq 10$', x=18, location='above right', use_arrow=True)
...
def plot_energy(data, sel_str):
    ...
    graph.histogram(n, bins)
    graph.add_pin(r'$T_{CC} \geq 10$', x=14.6, location='above right', use_arrow=True)
    n, bins = histogram(log10(false_energy), bins=linspace(14, 18, 51))
    graph.histogram(n, bins, linestyle='gray')
    graph.add_pin('uncorrelated', x=15.5, location='above right', use_arrow=True)
...
def reconstruct_shower_sizes(data, tcc):
    reconstruction = KascadeCoreReconstruction(data, '/core', overwrite=True)
    reconstruction.reconstruct_core_positions('/hisparc/cluster_kascade/station_601', '/kascade', tcc)
    ...
    def reconstruct_core_positions(self, hisparc_group, kascade_group, tcc):
        ...
        for idx, tcc_value in pbar(zip(c_index[: self.N], tcc)):
            ...
            if tcc_value >= 10:
                ...
                self.store_reconstructed_event(hisparc_event, kascade_event, x, y, N)

        self.results_table.flush()

    def store_reconstructed_event(
        self,
        hisparc_event,
        kascade_event,
