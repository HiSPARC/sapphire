...
if USE_TEX:
    ...
    plt.rcParams['figure.figsize'] = [4 * x for x in (1, 2.0 / 3)]
...
class ReconstructionEfficiency:
    ...
    def calc_charged_spectrum(self, x, y, p_gamma, p_landau):
        ...
        y_gamma_trunc = np.where(x <= 3 * max_pos, y_gamma, 0.0)
        ...
        y_charged = np.array(y_charged_left.tolist() + y_charged_right.tolist())

        return y_charged

    def full_spectrum_fit(self, x, y, p0_gamma, p0_landau):
        p_gamma = self.fit_gammas_to_data(x, y, p0_gamma)
        p_landau = self.fit_conv_landau_to_data(x, y - self.gamma_func(x, *p_gamma), p0_landau)
    ...
    def plot_gamma_landau_fit(self):
        ...
        p_gamma, p_landau = self.full_spectrum_fit(x, n, (1.0, 1.0), (5e3 / 0.32, 3.38 / 5000, 1.0))
        print('FULL FIT')
    ...
    def plot_spectrum_fit_chisq(self):
        ...
        x_trunc = x.compress((x >= LOW) & (x < HIGH))
        n_trunc = n.compress((x >= LOW) & (x < HIGH))
        fit_trunc = fit.compress((x >= LOW) & (x < HIGH))
    ...
    def plot_landau_and_gamma(self, x, p_gamma, p_landau):
        gammas = self.gamma_func(x, *p_gamma)
        gamma_trunc = np.where(x * VNS <= 21, gammas, 0.0)
    ...
    def fit_gammas_to_data(self, x, y, p0):
        condition = (x >= LOW) & (x < 2000)
        x_trunc = x.compress(condition)
        y_trunc = y.compress(condition)
        popt, pcov = optimize.curve_fit(self.gamma_func, x_trunc, y_trunc, p0=p0, sigma=np.sqrt(y_trunc))
        return popt
    ...
    def fit_conv_landau_to_data(self, x, y, p0):
        popt = optimize.fmin(self.scintillator.residuals, p0, (x, y, 4500, 5500), disp=0)
        return popt

    def fit_complete(self, x, y, p_gamma, p_landau):
        p0 = list(p_gamma) + list(p_landau)
        popt = optimize.fmin(self.complete_residuals, p0, (self.scintillator, x, y, LOW, HIGH), maxfun=100000, disp=0)
    ...
    def constrained_fit_complete(self, x, y, p_gamma, p_landau):
        ...
        N_landau = p_landau[0]
        popt = optimize.fmin(
            self.constrained_complete_residuals,
            (N_gamma, N_landau),
            (self.scintillator, x, y, p_gamma, p_landau, LOW, HIGH),
            maxfun=100000,
            disp=0,
        )
        p_gamma[0] = popt[0]
    ...
    def complete_residuals(self, par, scintillator, x, y, a, b):
        ...
        y_trunc = np.where(y_trunc != 0.0, y_trunc, 1.0)

        chisquared = ((y_trunc - y_exp_trunc) ** 2 / y_trunc).sum()
        return chisquared

    def constrained_complete_residuals(self, par, scintillator, x, y, p_gamma, p_landau, a, b):
    ...
    def plot_detection_efficiency(self):
        ...
        popt = self.full_fit_on_data(integrals, (1.0, 1.0, 5e3 / 0.32, 3.38 / 5000, 1.0))
        ...
        plt.figure()
        plt.errorbar(x, y, yerr, fmt='o', label='data', markersize=3.0)

        popt, pcov = optimize.curve_fit(self.conv_p_detection, x, y, p0=(1.0,))
        ...
        graph.plot(x2, self.conv_p_detection(x2, *popt), mark=None, linestyle='dashed')
