...
def plot_uncertainty_core_distance(table):
    ...
    title(
        ...
        % (N, DN, rad2deg(DTHETA), LOGENERGY - DLOGENERGY, LOGENERGY + DLOGENERGY),
    )
    ylim(ymin=0)
...
def plot_theta_reconstruction_results_for_MIP(table, N):
    ...
    H, x_edges, y_edges = histogram2d(rad2deg(sim_theta), rad2deg(r_theta), bins=bins)
    graph.histogram2d(H, x_edges, y_edges, type='reverse_bw')
