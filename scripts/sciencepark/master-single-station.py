...
class Master:
    ...
    datetimerange = (datetime.datetime(2012, 2, 1), datetime.datetime(2012, 2, 2))
    ...
    def store_coincidences(self):
        ...
        if '/coincidences' not in self.data:
            ...
            self.c_index = []
            self.coincidences = self.data.create_table(group, 'coincidences', storage.Coincidence)
            self.observables = self.data.create_table(group, 'observables', storage.EventObservables)

            for coincidence in pbar(self.data.root.c_index):
                self.store_coincidence(coincidence)

            c_index = self.data.create_vlarray(group, 'c_index', tables.UInt32Col())
    ...
    def store_coincidence(self, coincidence):
        ...
        for index in coincidence:
            ...
            idx = self.store_event_in_observables(event, coincidence_id, station_id)
