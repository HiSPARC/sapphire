...
class Master:
    ...
    datetimerange = (datetime.datetime(2012, 1, 1), datetime.datetime(2012, 1, 2))
    ...
    def download_data(self):
        ...
        for station, group_path in zip(self.stations, self.station_groups):
            if group_path not in self.data:
                print('Downloading data for station', station)
                download_data(self.data, group_path, station, start, end, get_blobs=True)

    def clean_data(self):
        for group in self.station_groups:
            group = self.data.get_node(group)
            attrs = group._v_attrs
            if 'is_clean' not in attrs or not attrs.is_clean:
    ...
    def clean_events_in_group(self, group):
        ...
        tmptable = self.data.create_table(group, 't__events', description=events.description)
    ...
    def search_coincidences(self):
        ...
        if '/coincidences' not in self.data:
            coincidences = sapphire.analysis.coincidences.Coincidences(self.data, '/coincidences', self.station_groups)
    ...
    def reconstruct_core_position(self):
        ...
        if '/core_reconstructions' not in self.data:
            reconstruction = CoreReconstruction(self.data, self.stations, '/core_reconstructions')
    ...
    def determine_station_offsets(self, overwrite=False):
        ...
        if offsets_group in self.data and not overwrite:
            ...
            for station_id, station_group in enumerate(station_groups):
                ...
                x = (bins[:-1] + bins[1:]) / 2
                popt, pcov = curve_fit(gauss, x, y, p0=(len(dt), 0, 100.0))
                print(popt)
                self.station_offsets.append(popt[1])

            ref_idx = self.station_groups.index(ref_group)
            self.station_offsets.insert(ref_idx, 0.0)
...
class ClusterDirectionReconstruction(DirectionReconstruction):
    ...
    reconstruction_coincidence_description = {
        'id': tables.UInt32Col(),
        'N': tables.UInt8Col(),
    }
    ...
    def _create_reconstruction_group_and_tables(self, results_group, overwrite):
        ...
        stations_description = {'s%d' % u: tables.BoolCol() for u in self.stations}

        description = self.reconstruction_description
        description.update(stations_description)
        self.reconstruction = self.data.create_table(group, 'reconstructions', description)

        description = self.reconstruction_coincidence_description
        description.update(stations_description)
        self.reconstruction_coincidences = self.data.create_table(group, 'coincidences', description)
    ...
    def reconstruct_angle(self, event, offsets=None):
        ...
        phi = arctan2((dt2 * r1 * cos(phi1) - dt1 * r2 * cos(phi2)), (dt2 * r1 * sin(phi1) - dt1 * r2 * sin(phi2)) * -1)
    ...
    def reconstruct_cluster_angle(self, events):
        ...
        for event in events:
            ...
            arrival_times = [event[u] for u in ('t1', 't2', 't3', 't4') if event[u] != -999.0]
            arrival_times.sort()
    ...
    def store_reconstructed_event_from_single_station(self, coincidence, event, reconstructed_theta, reconstructed_phi):
    ...
    def store_reconstructed_event_from_cluster(
        self,
        coincidence,
        events,
        index_group,
