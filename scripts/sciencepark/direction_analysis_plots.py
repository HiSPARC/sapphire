...
def plot_sciencepark_cluster():
    ...
    artist.utils.save_data([x_list, y_list], suffix='detectors', dirname='plots')
    artist.utils.save_data([stations, x_stations, y_stations], suffix='stations', dirname='plots')
...
def calc_direction_single_vs_cluster(data, station, cluster, limit=None):
    ...
    return (
        array(theta_station).flatten(),
        array(phi_station).flatten(),
        array(theta_cluster).flatten(),
        array(phi_cluster).flatten(),
    )
...
def calc_direction_single_vs_single(data, station1, station2):
    ...
    return (
        array(theta_station1).flatten(),
        array(phi_station1).flatten(),
        array(theta_station2).flatten(),
        array(phi_station2).flatten(),
    )
...
def plot_direction_single_vs_cluster(data, station, cluster):
    ...
    theta_station, phi_station, theta_cluster, phi_cluster = calc_direction_single_vs_cluster(data, station, cluster)
...
def plot_N_vs_R(data):
    ...
    if 'c_x' in globals():
        ...
        for coincidence_events in c_index:
            stations = [observables[u]['station_id'] for u in coincidence_events]
            stations_in_coincidence.append(stations)

        c_x = []
        c_y = []
        for station1, station2 in itertools.combinations(station_ids, 2):
            condition = [station1 in u and station2 in u for u in stations_in_coincidence]
            N = sum(condition)
...
def plot_fav_single_vs_cluster(data):
    ...
    for n, station in enumerate(cluster, 1):
        theta_station, phi_station, theta_cluster, phi_cluster = calc_direction_single_vs_cluster(
            data,
            station,
            cluster,
            2000,
        ...
        H, x_edges, y_edges = histogram2d(rad2deg(phi_station), rad2deg(phi_cluster), bins=bins)
        ...
        H, x_edges, y_edges = histogram2d(rad2deg(theta_station), rad2deg(theta_cluster), bins=bins)
...
def plot_fav_single_vs_single(data):
    ...
    for i in range(len(cluster)):
        for j in range(len(cluster)):
            ...
            theta_station1, phi_station1, theta_station2, phi_station2 = calc_direction_single_vs_single(
                data,
                station1,
                station2,
            )
            ...
            if i > j:
                ...
                bins = linspace(-180, 180, 37)
                H, x_edges, y_edges = histogram2d(rad2deg(phi_station1), rad2deg(phi_station2), bins=bins)
                graph.histogram2d(j, i, H, x_edges, y_edges, 'reverse_bw')
                graph.set_label(j, i, r'$\phi$', 'upper left', style='fill=white')
            elif i < j:
                ...
                bins = linspace(0, 45, 46)
                H, x_edges, y_edges = histogram2d(rad2deg(theta_station1), rad2deg(theta_station2), bins=bins)
                graph.histogram2d(j, i, H, x_edges, y_edges, 'reverse_bw')
                graph.set_label(j, i, r'$\theta$', 'upper left', style='fill=white')
...
def plot_fav_uncertainty_single_vs_cluster(data):
    ...
    for n, station in enumerate(cluster, 1):
        ...
        for low, high in zip(bins[:-1], bins[1:]):
            sel_phi_c = phi_cluster.compress((low <= theta_station) & (theta_station < high))
            sel_phi_s = phi_station.compress((low <= theta_station) & (theta_station < high))
            sel_theta_c = theta_cluster.compress((low <= theta_station) & (theta_station < high))
            sel_theta_s = theta_station.compress((low <= theta_station) & (theta_station < high))
        ...
        ephi, etheta = [], []
        for theta in ex:
            ephi.append(calc_phi_error_for_station_cluster(theta, n, cluster_ids))
            etheta.append(calc_theta_error_for_station_cluster(theta, n, cluster_ids))
...
def plot_fav_uncertainty_single_vs_single(data):
    ...
    for i in range(len(cluster)):
        for j in range(len(cluster)):
            ...
            x, y, y2 = [], [], []
            for low, high in zip(bins[:-1], bins[1:]):
                sel_phi_c = phi_station2.compress((low <= theta_station1) & (theta_station1 < high))
                sel_phi_s = phi_station1.compress((low <= theta_station1) & (theta_station1 < high))
                sel_theta_c = theta_station2.compress((low <= theta_station1) & (theta_station1 < high))
                sel_theta_s = theta_station1.compress((low <= theta_station1) & (theta_station1 < high))
                dphi = sel_phi_s - sel_phi_c
                dtheta = sel_theta_s - sel_theta_c
                # make sure phi, theta are between -pi and pi
                dphi = (dphi + pi) % (2 * pi) - pi
...
def calc_phi_error_for_station_cluster(theta, station, cluster):
    ...
    err_total = sqrt(STATION_TIMING_ERR**2 * err_single + CLUSTER_TIMING_ERR**2 * err_cluster)
...
def calc_phi_error_for_station_station(theta, station1, station2):
    ...
    # errors are already squared!!
    err_total = sqrt(STATION_TIMING_ERR**2 * err_single1 + STATION_TIMING_ERR**2 * err_single2)
    return mean(err_total)
...
def hist_fav_single_stations(data):
    ...
    for n, station in enumerate([501, 503, 506], 1):
        ...
        subplot(2, 3, n)
        N, bins, patches = hist(rad2deg(phi), bins=linspace(-180, 180, 21), histtype='step')
        x = (bins[:-1] + bins[1:]) / 2
        f = lambda x, a: a
        ...
        subplot(2, 3, n + 3)
        N, bins, patches = hist(rad2deg(theta), bins=linspace(0, 45, 21), histtype='step')
        xlabel(r'$\theta$')
        legend([station], loc='lower right')
        locator_params(tight=True, nbins=4)
        axis('auto')
